        ...
        while i < len(blocks):
            ...
            while i < len(blocks) and blocks_on_slide < max_blocks_per_slide:
                ...
                textbox = slide.shapes.add_textbox(
                    margin_left, y, slide_width - 2 * margin_left, box_height
                )
                textbox.text = text
...
def upload_pdf():
    ...
    if file and allowed_file(file.filename):
        ...
        threading.Thread(
            target=background_conversion, args=(upload_id, pdf_path, pptx_path)
        ).start()
...
def check_status(upload_id):
    ...
    resp = {"status": status, "progress": progress, "uploadId": upload_id}
    if status == "completed":
...
def download_file(upload_id):
    ...
    return send_file(
        pptx_path,
        as_attachment=True,
        download_name=info.get("filename", "converted-presentation.pptx"),
    )
